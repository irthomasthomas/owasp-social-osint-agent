...
from ..utils import NormalizedPost, NormalizedProfile, UserData, extract_and_resolve_urls, get_sort_key
...
class HackerNewsFetcher(BaseFetcher):
    ...
    def _get_or_fetch_profile(self, username: str, cached_data: Optional[UserData], force_refresh: bool, **kwargs) -> Optional[NormalizedProfile]:
        ...
        try:
            # We fetch from the official Firebase API to get Karma and About text
            ...
            with httpx.Client(timeout=20.0) as client:
                ...
                return NormalizedProfile(
                    ...
                    display_name=username, bio=d.get("about", ""),
                    created_at=get_sort_key({"ts": d.get("created")}, "ts"),
                    profile_url=f"https://news.ycombinator.com/user?id={username}",
                    metrics={"karma": d.get("karma", 0), "submitted_count": len(d.get("submitted", []))}
                )
    ...
    def _fetch_posts(self, username: str, profile: NormalizedProfile, needed_count: int, processed_ids: set, **kwargs) -> List[NormalizedPost]:
        new_posts = []
        try:
            # Algolia is superior for searching history by author
