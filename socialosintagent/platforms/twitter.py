...
def fetch_data(
    ...
    since_id, pagination_token = None, None
    ...
    if is_incremental_update and existing_tweets:
    ...
    try:
        ...
        tweets_to_process = list(existing_tweets) if is_load_more else []
        
        while len(tweets_to_process) < fetch_limit:
            page_limit = min(fetch_limit - len(tweets_to_process), 100)
            if page_limit <= 0 and not is_incremental_update: break
            
            tweets_response = client.get_users_tweets(
                id=user_id, max_results=page_limit if page_limit > 0 else 100,
            ...
            if tweets_response.data:
                tweets_to_process.extend(tweets_response.data)
        ...
        for tweet_obj in tweets_to_process:
