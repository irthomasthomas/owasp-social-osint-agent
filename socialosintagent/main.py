...
""",
    ...
    parser.add_argument(
        "--stdin", action="store_true", help="Read analysis request from stdin as JSON."
    )
    parser.add_argument(
        "--format",
        choices=["json", "markdown"],
        default="markdown",
        help="Output format for saving analysis reports.",
    )
    parser.add_argument(
        "--no-auto-save",
        action="store_true",
    ...
    parser.add_argument(
        "--unsafe-allow-external-media",
        action="store_true",
        help="Security: Allow downloading media from domains outside known social media CDNs (e.g. personal servers).",
    )
    ...
    try:
        ...
        config = AgentConfig(
            ...
            unsafe_allow_external_media=args.unsafe_allow_external_media,
            no_auto_save=args.no_auto_save,
        ...
        agent_instance = SocialOSINTAgent(
            config, cache_manager, llm_analyzer, client_manager
        )
        ...
        error_console.print(
            "Ensure necessary API keys and platform credentials/URLs are correctly set."
        )
        sys.exit(1)
    except Exception as e:
        logging.getLogger("SocialOSINTAgent").critical(
